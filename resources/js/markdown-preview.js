...
window.preview = function(element) {
  if (!element.previewWindow) {
    ...
    element.previewWindow.text.innerHTML = marked(DOMPurify.sanitize(element.value));
...
function update(event) {
  event.target.previewWindow.text.innerHTML = marked(DOMPurify.sanitize(event.target.value));
}
