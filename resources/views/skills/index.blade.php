...
@section('content')
    <x-container>
        <x-card class="space-y-4">
            <a class="flex max-w-fit space-x-2 items-center font-bold text-gray-600 dark:text-gray-300" href="{{ route('skills.create') }}">
            ...
            @if ($skills->isNotEmpty())
                @foreach ($skills as $skill)
                    <x-skill :skill="$skill">
                        ...
                        <x-slot name="list">
                            <a class="px-4 py-2 flex space-x-2 items-center font-bold text-gray-600 dark:text-gray-300" href="{{ route('advantages.create', ['skill' => $skill]) }}">
                                <div class="far fa-plus-square text-xl"></div>
                        ...
                        @foreach($skill->advantages as $advantage)
                            <x-slot :name="'advantage_' . $advantage->id">
                                <div class="col-span-1 p-2 flex gap-2 text-gray-600 dark:text-gray-300">
                                    <a class="fas fa-edit text-xl"
                                    ...
                                    <form method="POST" action="{{ route('advantages.destroy', ['skill' => $skill, 'advantage' => $advantage]) }}">
                                        ...
                                        <a class="fas fa-trash cursor-pointer text-xl"
                                           onclick="event.preventDefault();this.closest('form').submit();"></a>
                                    </form>
                                </div>
                            </x-slot>
