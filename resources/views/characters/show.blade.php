            ...
            @if(count($character->narrativeCrafts))
              ...
              @foreach($character->narrativeCrafts as $craft)
                <h2 id="accordion-collapse-heading-{{$loop->iteration}}">
                  <button type="button" class="flex items-center justify-between w-full p-2 font-medium text-left text-gray-500 border border-gray-200 {{$loop->last ? "" : "border-b-0"}}" data-accordion-target="#accordion-collapse-body-{{$loop->iteration}}" aria-expanded="true" aria-controls="accordion-collapse-body-{{$loop->iteration}}">
